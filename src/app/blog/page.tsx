            ...
            {posts.map((post, i) => (
              ...
              >
                <div className="flex items-center gap-3 mb-4">
                  <span className="px-3 py-1 rounded-full bg-gradient-to-r from-reiki-500/20 to-heal-400/20 text-xs font-medium text-reiki-600">
                    {post.category}
                  </span>
      ...
      <section className="relative py-24 gradient-spiritual">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 text-center reveal">
          ...
          <form className="flex flex-col sm:flex-row gap-4" onSubmit={(e) => e.preventDefault()}>
            <label htmlFor="newsletter-email" className="visually-hidden">{t("blog.newsletter.placeholder")}</label>
            <input
              id="newsletter-email"
              type="email"
