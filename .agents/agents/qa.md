...
17. **Mobile-first layout**: en páginas con split layout (/agendar, /contacto), verifica que a 375px el formulario o la acción principal aparezca ANTES que la imagen decorativa. Usa `playwright_browser_resize 375 812` y verifica el orden de los elementos en el DOM con `playwright_browser_snapshot`. Si la imagen está antes que el form en mobile → `ISS-LAYOUT` Media.
18. **Contraste en glassmorphism**: en formularios con fondos translúcidos (`backdrop-blur`, `bg-white/90`), verifica que las etiquetas (`<label>`) y el texto de los inputs tengan contraste suficiente contra el fondo. Inspecciona visualmente con screenshot. Si el texto se vuelve ilegible por el fondo → `ISS-A11Y-CONTRAST` Alta.
19. **Prefers-reduced-motion**: verifica que las animaciones (FadeInWrapper, glow hover, smooth scroll) se respetan cuando el usuario tiene `prefers-reduced-motion: reduce`. Evalúa con: `playwright_browser_evaluate(() => window.matchMedia('(prefers-reduced-motion: reduce)').matches)`. Si el portal tiene animaciones activas con esta preferencia → `ISS-A11Y-MOTION` Alta.
20. **Smooth scroll en touch devices**: el smooth scroll (Lenis) debe desactivarse en dispositivos táctiles para no competir con el scroll nativo. Verifica en el código que LenisProvider tiene `ontouchstart` check o `smoothTouch: false`. Si no → `ISS-PERF-SCROLL` Media.
21. **No overlap con header fijo**: en TODAS las páginas, verifica que el primer elemento de contenido visible (título h1, badge, o texto) NO se superponga con el header fijo. Mide con: `playwright_browser_evaluate(() => { const h = document.querySelector('header,[role=banner]'); const t = document.querySelector('h1'); return { headerBottom: h?.getBoundingClientRect().bottom, titleTop: t?.getBoundingClientRect().top, overlap: (t?.getBoundingClientRect().top||0) < (h?.getBoundingClientRect().bottom||0) }; })`. Si `overlap === true` → `ISS-LAYOUT-OVERLAP` Alta.
